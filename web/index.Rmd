...
ambulance <- readr::read_csv("../Data/ambulance.csv") |> 
  mutate(year = year(date), week = week(date),
         year_week = str_glue("{year}_{sprintf('%02d', week)}")) |> 
  filter(year_week >= "2022_41") |> 
  mutate(city = factor(city, levels = c("福岡市消防局", "北九州市消防局")))
nurse <- readr::read_csv("../Data/nurse.csv") |> 
  mutate(year = year(date), week = week(date),
         year_week = str_glue("{year}_{sprintf('%02d', week)}")) |> 
  filter(year_week >= "2022_41") |> 
...
```{r}
plot_ly() |> 
  add_lines(data = ambulance, x = ~date, y = ~value, color = ~city) |> 
...
```{r}
plot_ly() |> 
  add_lines(data = nurse, x = ~date, y = ~value, color = ~type) |> 
  layout(legend = list(x = 0.05, y = 0.95),
         xaxis = list(title = NA), yaxis = list(title = NA),
